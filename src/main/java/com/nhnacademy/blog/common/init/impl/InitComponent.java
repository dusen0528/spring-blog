...
public class InitComponent implements Initializeable {
    ...
    public void initialize(Context context) {
        ...
        /**
         * 1 @Componet annotation을 기준으로 'com.nhnacademy.blog' package 하위에 있는 class를 scan 합니다.
        ...
        List<ClassWrapper> classWrappers =  ReflectionUtils.classScanByAnnotated("com.nhnacademy.blog", Component.class);
         ...
         /*2 componentList, serviceList,repositoryList 초기화 합니다.
        ...
        for (ClassWrapper<Component> classWrapper : classWrappers) {
            ...
            //3 classWrapper.getClazz()의 타입이 interface이면 continue;를 호출 합니다.
            //즉 @Component, @Repository, @Service를 정의한 Interface는 제외하려고 합니다.
            if(classWrapper.getClazz().isInterface()){
                continue;
            }

            /*4 classWrapper.getClazz()를 각각의 annotation의 조건에 맞게 분리 합니다.
        ...
        /*5  @Compoent로 선언된 class의 인스턴스를  createInstance() 메서드를 이용해서  생성합니다.
        ...
        /*6  @Repository로 선언된 class의 인스턴스를  createInstance() 메서드를 이용해서  생성합니다.
            - Bean 생성및 등록(2순위)
            - beanName이 존재하지 않는 경우 classNameToBeanName() 메서드를 호출해서 이름을 설정합니다.
         */
        ...
        /*7  @Service로 선언된 class의 인스턴스를  createInstance() 메서드를 이용해서  생성합니다.
            - Bean 생성및 등록(3순위)
            - beanName이 존재하지 않는 경우 classNameToBeanName() 메서드를 호출해서 이름을 설정합니다.
         */
    ...
    public static String classNameToBeanName(Class<?> clazz) {
        //8 classNameToBeanName()를 구현 합니다.
    ...
    private void createInstance(Context context, Class targetClass, String beanName) {
        ...
        //9 객체 생성에 사용할 Constructor를 ReflectionUtils.findFirstConstructor()를 사용해서 구합니다.
        Constructor constructor = ReflectionUtils.findFirstConstructor(targetClass);

        /*10 constructor에 해당되는 parameter 순서대로 Object[] 배열형태로 반환 합니다.
        ...
        /*11 parameters를 이용해서 객체를 생성하고, 생성된 객체는 context.registerBean() 메서드를 이용해서 Bean(객체)로 등록 합니다.
         - 모든 Stereotype Bean이 생성되어 Application Context에 등록되었습니다.
